...
def transform():
    ...
    log(logfile, "TransformacionInicia")
    ...
    df_customers = pd.read_sql_query("""SELECT 
        CustomerId,  FirstName, LastName, COALESCE(Company, 'N/A') as Company, 
        Address,  City, COALESCE(State, 'N/A') as State, Country, COALESCE(PostalCode, 'N/A') as PostalCode, 
        COALESCE(Phone, 'N/A') as Phone, COALESCE(Fax, 'N/A') as Fax, 
        Email FROM customers; """, con=engine.connect())
    df_employees = pd.read_sql_query("""SELECT 
        EmployeeId,  LastName,  FirstName, Title, 
        BirthDate, HireDate, City,  State, Country, PostalCode,  Phone, 
        Fax, Email FROM employees;
        """, con=engine.connect())
    df_location = pd.read_sql_query("""SELECT 
        CustomerId as LocationId,  Address, City, COALESCE(State, 'N/A') as State, 
        Country,  COALESCE(PostalCode, 'N/A') as PostalCode  FROM customers;
        """, con=engine.connect())
    df_tracks = pd.read_sql_query("""SELECT t.TrackId, t.Name as TrackName, al.Title as Album, g.Name as Genre, mt.Name as MediaType, COALESCE(t.Composer, 'N/A') as Composer, t.Milliseconds, t.Bytes, t.UnitPrice as Price 
        FROM tracks t JOIN albums al on t.AlbumId = al.AlbumId JOIN genres g on t.GenreId = g.GenreId
        JOIN media_types mt on mt.MediaTypeId = t.MediaTypeId ORDER BY 1;
        """, con=engine.connect())
    df_artists = pd.read_sql_query("""select ar.ArtistId, ar.Name as ArtistName from artists ar;""", con=engine.connect())

    log(logfile, "TransformacionFinaliza")
    return df_fact_sales,df_customers,df_employees,df_location,df_tracks,df_artists
   
def load():
    """ Connect to the PostgreSQL database server """
    conn_string = 'postgresql://postgres:caiza123@localhost/DW_Sales_Music'
    db = create_engine(conn_string)
    conn = db.connect()
    try:
        log(logfile, "CargaInicia")
